...
<AccordionGroup>
  ...
  <Accordion title="YouTube link">
    Optional. A YouTube video URL (`youtube.com/watch?v=…` or `youtu.be/…`) to embed a video in the post. The processed `embed_url` and `embed_provider` are computed from this value at read time.
  </Accordion>
  <Accordion title="Audio">
    Optional. Upload an audio file (`audio_url` and `audio_filename` columns) to attach to the post — useful for podcast episodes or audio-category posts. SoundCloud track URLs are also supported as an embed source via the same embed pipeline as YouTube.
  </Accordion>
