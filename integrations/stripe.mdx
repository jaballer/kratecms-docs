...
| Stripe event | What KrateCMS does |
|---|---|
| `customer.subscription.deleted` | Removes Pro access from the member's account |
| `invoice.payment_failed` | Flags the subscription as past due |
| `invoice.payment_succeeded` | Confirms the subscription is active |
| `customer.subscription.updated` | Syncs plan changes (e.g. monthly → annual) |
