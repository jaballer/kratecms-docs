...
| **Require special characters** | When enabled, passwords must include at least one special character (such as `!`, `@`, `#`, `$`). Off by default. |
...
<Note>
  Your lockout settings sit on top of a platform-wide rate limit that KrateCMS applies to sign-in, registration, password-reset, and forgot-password requests. That limit is per IP address and shared across every site on the platform, so it can't be sidestepped by spreading attempts across different sites — and it applies however generous your own lockout setting is.
</Note>
