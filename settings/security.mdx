...
When a user can't get into their account, you have several recovery actions available from the user detail page in **Admin → Users**:

- **Send password reset** — emails the user a reset link without requiring them to use the public "Forgot password" flow
- **Resend invite** — re-sends the welcome / set-password email if the original was lost or expired
- **Force logout** — immediately terminates every active session for the user across every device
- **Enable / disable account** — toggle whether the user can sign in at all; disabling kills active sessions on the next request

See [Inviting and managing users](/settings/users) for the full workflow.

## Login activity

Open **Admin → Activity → Login** (`/admin/activity/login`) — available to manager-and-above when the platform feature flag is enabled — to review every login attempt against your site. Each row shows the user, IP address, success or failure, and timestamp. Use it to:

- Spot brute-force attempts targeting a specific account
- Confirm a user successfully signed in after a password reset
- Audit who has accessed the admin from where over the recent past
